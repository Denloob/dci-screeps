...
Room.prototype.displayData =
    function () {
        ...
        this.visual.text(`CPU:`, 1, 3, {color: cpuCollor, font: 0.8, align: LEFT}); this.visual.text(`${ cpuPersentage }%`, 4, 3, {color: cpuCollor, font: 0.8, align: LEFT});
        let bucketPersentage = Math.round( Game.cpu.bucket/10000*100 *10)/10
        let bucketCollor = cpuPersentage < 20 ? 'red': 'green'
        this.visual.text(`BKT:`, 1, 4, {color: bucketCollor, font: 0.8, align: LEFT}); this.visual.text(`${ bucketPersentage }%`, 4, 4, {color: bucketCollor, font: 0.8, align: LEFT});
        let gclPersentage = Math.round( Game.gcl.progress/Game.gcl.progressTotal*100 *10)/10
        this.visual.text(`GCL:`, 1, 5, {color: '#069090', font: 0.8, align: LEFT}); this.visual.text(`${ gclPersentage }%`, 4, 5, {color: '#069090', font: 0.8, align: LEFT});
        this.visual.text(`LVL:`, 1.005, 6, {color: '#069090', font: 0.8, align: LEFT}); this.visual.text(`${ Game.gcl.level }`, 4, 6, {color: '#069090', font: 0.8, align: LEFT});
        this.visual.text(`Viewing: [ ${ this.name } ]`, 1, 8, {color: '#DEDEDE', font: 0.8, align: LEFT});
        ...
        if (this.find(FIND_MY_SPAWNS).length) listOfRoles.forEach(role => {
            ...
            else if (role.startsWith('@@')) { listOfLimitedRoles.push(role); return; }
        ...
        this.visual.rect(0, 10, 9, lastRectSizes[0], {fill: fillRect, stroke: gray}); // creeps info rect
        this.visual.text(`Creeps Info`, 1, 11, {color: 'white', font: 0.8, align: LEFT});
        ...
        this.visual.text(`Limited Roles`, 1, 13, {color: limitedRolesCollor, font: 0.8, align: LEFT});
                    ...
                    switch (displayData.displayLDH) {
                        case 'ROOM':
                            if (!displayedLDHs.includes(roomName)) {
                                ...
                                neededCreeps = Object.values(Object.keys(data).filter(function(k) {
        ...
        // general Info rect
        this.visual.rect(40, 0, 9, lastRectSizes[1], {fill: fillRect, stroke: gray});
        this.visual.text('General Info', 41, 1.3, {color: 'white', font: 0.8, align: LEFT});
        ...
        // room data rect
        this.visual.rect(40, endIndexCopy, 9, lastRectSizes[2], {fill: fillRect, stroke: gray});
        endIndex+=1.5;
        this.visual.text('Room Data:', 41, endIndex, {color: 'white', font: 0.8, align: LEFT});
        endIndex+=2;
        ...
        this.visual.text(`Energy in The Room:`, 41, endIndex, {color: 'white', font: 0.8, align: LEFT}); endIndex++;
        let energyPersentage = Math.round( this.energyAvailable/this.energyCapacityAvailable*100 *10)/10
        this.visual.text(`${this.energyAvailable}/${this.energyCapacityAvailable}  (${energyPersentage}%)`, 41, endIndex, {color: energyInTheRoomCollor, font: 0.8, align: LEFT}); endIndex++;
        endIndex++;
        
        this.visual.text(`Room Level:`, 41, endIndex, {color: 'white', font: 0.8, align: LEFT}); this.visual.text(`${this.controller.level}`, 46, endIndex, {color: 'white', font: 0.8, align: LEFT}); endIndex++;
        let controllerProgressPersentage = Math.round( this.controller.progress/this.controller.progressTotal*100 *10)/10
        this.visual.text(`Progress:`, 41, endIndex, {color: 'white', font: 0.8, align: LEFT}); this.visual.text(`${controllerProgressPersentage}%`, 46, endIndex, {color: 'white', font: 0.8, align: LEFT}); endIndex++;
...
Room.prototype.globalOption =
    function (name, data, roomList=false) {
        if (name != undefined && data != undefined) {
...
Room.prototype.globalHardOption =
    function (name, data, roomList=false) {
        if (name != undefined && data != undefined) {
            if (!roomList) roomList = Object.keys(Game.rooms).filter(roomName => Game.rooms[roomName].controller && Game.rooms[roomName].controller.my)
            for (roomName of roomList) {
