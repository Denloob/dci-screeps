...
var statsConsole = {
  ...
  displayHistogram: function (width = 100, height = 20) {
    ...
    let output = asciiChart.chart(
      Memory.stats.__cpu.slice(0, Math.floor(width / 3)).reverse(),
      {
        width: width,
        height: height,
      }
  ...
  displayStats: function (opts = {}) {
    ...
    let lineStat = [
      ...
      cpuAverage && cpuLimit
        ...
        : 0,
      cpuBucket ? cpuBucket.toFixed(0).toString() : 0,
    ];
    ...
    let secondLineName = [
      "GCL" +
        Game.gcl.level +
        " - " +
        ((gclProgress / Game.gcl.progressTotal) * 100).toFixed(0) +
        "%",
    ];
    let secondLineStat = [
      ((gclProgress / Game.gcl.progressTotal) * 100).toFixed(2) + percent,
    ];
    if (useProgressBar === "yes") {
      secondLineStat = [
        _.repeat(
          progressBar,
          (gclProgress / Game.gcl.progressTotal) * (boxWidth / 4 - 2)
        ),
      ];
    }
  ...
  geohash: function () {
    // Get creep location and save as geohash
  ...
  log: function (message, color = "#ffffff") {
    Memory.stats.logs.push([Game.time + ": " + message, color]);
  },
  /**
   ...
   * @api public
   */
  displayLogs: function (opts = {}, logs = Memory.stats.logs) {
    let totalWidth = opts.width || 100;
    let maxHeight = opts.height || 10;
    ...
    let boxHeight = Math.min(logs.length, maxHeight);
    ...
    for (let i = 0; i < boxHeight; i++) {
      // Y coordinate |
      let color = Memory.stats.logs[i][(0, 1)];
      ...
      let htmlStart = `<log><span style='color: ${color}'><log>`.replace(
        /(\r\n|\n|\r)\t+|(\r\n|\n|\r) +|(\r\n|\n|\r)/gm,
        ""
      );
      let htmlFontStart = `<log><span style='color: ${color}'><log>`.replace(
        /(\r\n|\n|\r)\t+|(\r\n|\n|\r) +|(\r\n|\n|\r)/gm,
        ""
      );
      //let htmlEnd = "</font>";
      ...
      try {
        if (message.length > boxWidth) {
          // message is longer than boxWidth
          outputLog =
            outputLog +
            vbar +
            htmlStart +
          ...
          // message is longer than boxWidth * 2
          outputLog =
            outputLog +
            vbar +
            htmlStart +
          ...
          // If your message is longer that boxWidth you need to cut down on the length of your log messages.
          outputLog =
            outputLog +
            vbar +
            htmlStart +
        ...
        this.log(`${e}`, "#ff0000");
        this.log(`Try running delete Memory.stats.log[${i}]`, "#ff0000");
      }
    }
