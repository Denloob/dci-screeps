          ...
          if (roomMemory != undefined) {
            ...
            if (creep.room.terminal) {
              ...
              if (terminalMemory == undefined) {
                Memory.rooms[creep.room.name].terminal = {
                  enabled: false,
                  autoSell: [{ enabled: false, resource: undefined, minPrice: 0, maxDistance: -1 }],
                  from: [{ enabled: false, resource: undefined, order: false }],
                  to: [{ enabled: false, dealData: undefined, order: false }],
                };
              }
              storage = terminalMemory != undefined && terminalMemory.active && creep.room.terminal.store[RESOURCE_ENERGY] < 100000 ? creep.room.terminal : undefined;
            }
            // if we dont find one
