...
StructureTerminal.prototype.sendEnergy = function (dest, num = 100000) {
...
StructureTerminal.prototype.addTask =
  ...
  function (type, params, enableTerminal = true) {
    ...
    if (
      ...
      _.isEqual(Object.keys(Memory.rooms[this.room.name].terminal), [
        "enabled",
        "autoSell",
        "from",
        "to",
        ...
        case "auto":
          if (
            !_.isEqual(
              ...
              ["enabled", "resource", "minPrice", "maxDistance"].sort()
            )
          )
            ...
            return "error wrong params type";
        ...
        case "to":
          ...
          if (
            !(
              _.isEqual(
                Object.keys(params).sort(),
                ["enabled", "dealData", "order"].sort()
              ) ||
              !_.isEqual(
                Object.keys(params).sort(),
                ["enabled", "resource", "order"].sort()
              ) ||
              params.order
            )
          )
            ...
            !(
              typeof params["enabled"] === "boolean" &&
              (typeof params["resource"] === "string" ||
                _.isObject(params["dealData"])) &&
              typeof params["order"] === "boolean"
            )
          )
            return "error wrong params type";
        ...
        case "from":
          ...
          if (
            !(
              _.isEqual(
                Object.keys(params).sort(),
                ["enabled", "dealData", "order", "toStructure"].sort()
              ) ||
              !_.isEqual(
                Object.keys(params).sort(),
                ["enabled", "resource", "order", "toStructure"].sort()
              ) ||
              params.order
            )
          )
...
StructureTerminal.prototype.addOrder =
  ...
  function (
    ...
    if (
      ...
      let order = Game.market.createOrder(
        type,
        resourceType,
        price,
        totalAmount,
      ...
      if (order == OK) {
        if (type == ORDER_SELL)
          this.addTask(
            "to",
            { enabled: true, resource: resourceType, order: true },
            enableTerminal
          );
        else
          this.addTask(
...
StructureTerminal.prototype.processTasks = function () {
    ...
    if (terminalMemory.enabled) {
      ...
      terminalMemory.autoSell.forEach((task, index) => {
        ...
        if (
          ...
          if (task.enabled) {
            // find best offer
            let dealData = this.getBestOffer(
              task.resource,
              task.minPrice,
              task.maxDistance
            );
            if (dealData) {
              let sameResourceDeal = _.filter(
                terminalMemory.to,
                (o) =>
                  !o.order &&
                  _.isEqual(
                    o.dealData != undefined
                      ? o.dealData.resourceType
                      : undefined,
                    dealData.resourceType
                  )
              );
              ...
              this.addTask(
                "to",
                { enabled: true, dealData: dealData, order: false },
                true
              );
            }
          }
          let diedOrders = _.filter(
            terminalMemory.to,
            (o) =>
              !o.order &&
              (o.dealData == undefined ||
                Game.market.getOrderById(o.dealData.id) === null)
          );
      ...
      terminalMemory.to.forEach((task, index) => {
        ...
        if (task.enabled && task.order == false) {
          ...
          let tradeAmount =
            dealData.amount > this.store[dealData.resourceType] &&
            this.store[dealData.resourceType] > 10000
              ? this.store[dealData.resourceType]
              : dealData.amount;
          if (dealData.resourceType == RESOURCE_ENERGY) {
            ...
            tradeAmount =
              tradeAmount + transactionCost > this.store[dealData.resourceType]
                ? tradeAmount - transactionCost
                : tradeAmount;
          ...
          tradeAmount =
            dealData.amount > 10000
              ? Math.max(tradeAmount, 5000)
              : dealData.amount;
  ...
  // else (if room memory is not set properly)
    ...
    Memory.rooms[this.room.name].terminal = {
      enabled: false,
      autoSell: [
        { enabled: false, resource: undefined, minPrice: 0, maxDistance: -1 },
      ],
      from: [
        {
          enabled: false,
      ...
      to: [{ enabled: false, dealData: undefined, order: false }],
    };
  }
};
// create a new function for StructureSpawn
StructureTerminal.prototype.deal = function (orderId, amount) {
  ...
  if (
    deal == OK &&
    Memory.hardMemory &&
    typeof Memory.hardMemory.rooms &&
    typeof Memory.hardMemory.rooms[this.room.name] &&
    typeof Memory.hardMemory.rooms[this.room.name] === "object" &&
    _.isEqual(Object.keys(Memory.hardMemory.rooms[this.room.name]), [
      "notifyOnDisplayReset",
      "commentsOnDisplayReset",
      "notifyOnTerminalDeal",
    ])
  ) {
    if (Memory.hardMemory.rooms[this.room.name].notifyOnTerminalDeal) {
      let dealData = Game.market.getAllOrders({ id: orderId })[0];
      console.log(
        `terminal in room ${this.room.name} transferd ${amount} of ${
          dealData.resourceType
        } to ${dealData.roomName} for ${dealData.price}$ each so you earned ${
          dealData.price * amount
        } and payed ${Game.market.calcTransactionCost(
          amount,
          this.room.name,
          dealData.roomName
        )} energy fee`
  ...
  function (resource, bestPice, maxDistance, type = ORDER_BUY) {
    ...
    if (type == ORDER_BUY) {
      ...
      orderFilter = (order) =>
        ...
        order.type == type &&
        Game.map.getRoomLinearDistance(this.room.name, order.roomName, true) <=
          maxDistance &&
        order.price >= bestPice;
      // // }
    }
    ...
    let bestOrder = _.sortBy(orders, (order) => order.maxDistance)[
      orders.length - 1
    ];
