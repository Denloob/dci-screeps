...
StructureTower.prototype.main = function () {
  // find closest hostile creep the owner of which is not in the white list of the room
  let target = this.pos.findClosestByRange(FIND_HOSTILE_CREEPS, {
    filter: (c) => _.some(c.body, (b) => b.type == ATTACK || b.type == CLAIM || b.type == RANGED_ATTACK || b.type == HEAL),
  });
  if (target == undefined) this.pos.findClosestByRange(FIND_HOSTILE_CREEPS);
  ...
  if (target != undefined) {
    ...
    if (
      ...
      'Invader' /** || this.room.find(FIND_HOSTILE_CREEPS, { filter: (c) => !Memory.rooms[this.room.name].whiteList.includes(c.owner.username)}).length > 1 */
    ) {
      ...
      if (
        target.owner.username == 'Tigga' &&
        _.some(target.body, (b) => b.type == ATTACK || b.type == CLAIM || b.type == RANGED_ATTACK || b.type == HEAL) &&
        this.room.find(FIND_MY_SPAWNS).length
      ) {
  ...
  // if no one is found
    ...
    let structure = this.pos.findClosestByRange(FIND_STRUCTURES, {
      filter: (s) => s.hits < s.hitsMax && ((s.structureType != STRUCTURE_WALL && s.structureType != STRUCTURE_RAMPART) || s.hits < wallsLimit),
    });
