...
Room.prototype.displayData = function () {
  ...
  let roomSpawnMemory = this.find(FIND_MY_SPAWNS).length
    ? Memory.spawns[this.find(FIND_MY_SPAWNS)[0].name]
    : undefined;
    ...
    if (ldhMin != undefined) {
      ...
      ldhMinData.forEach((data) => {
        if (ldhMin[data] > 0) {
          ...
          if (!listOfRoles.includes(`$$LDH_${data}`))
            listOfRoles.push(`$$LDH_${data}`);
        }
      });
    }
  }
  ...
  let cpuPersentage =
    Math.round(((Game.cpu.getUsed() * 100) / Game.cpu.limit) * 10) / 10;
  let cpuCollor =
    cpuPersentage < 50
      ? "green"
      : cpuPersentage < 80
  ...
  let gclPersentage =
    Math.round((Game.gcl.progress / Game.gcl.progressTotal) * 100 * 10) / 10;
    ...
    listOfLimitedRoles.forEach((role, indent) => {
      ...
      if (neededCreeps == undefined || role.startsWith("@@")) {
        if (role.startsWith("LDH")) {
          let [type, roomName, sourceIndex, numberOfWorkParts] =
            role.split("_");
          role = roomName;
          let data = undefined;
              ...
              if (!displayedLDHs.includes(roomName + sourceIndex)) {
                ...
                numOfCreeps = _.sum(
                  ...
                  (c) =>
                    c.memory.role == "LDH" &&
                    c.memory.target == roomName &&
                    c.memory.sourceIndex == sourceIndex &&
                    c.memory.home == this.name
                );
                // create copy of ldhMin
                data = Object.assign({}, ldhMin);
          ...
          if (role == "MH") {
            ...
            let needMineralWorker =
              roomExtractor != undefined &&
              roomMineral != undefined &&
              roomMineral.mineralAmount > 0;
            neededCreeps = needMineralWorker
              ? roomSpawnMemory.minCreeps[role]
              : 0;
          } else if (role == "TM") {
            if (
              roomMemory.terminal &&
              _.isEqual(Object.keys(roomMemory.terminal), [
                "enabled",
                "autoSell",
                "from",
                "to",
              ...
              (_.some(roomMemory.terminal.to, { enabled: true }) ||
                _.some(roomMemory.terminal.from, { enabled: true }))
            ) {
      ...
      if (neededCreeps != undefined) {
        ...
        this.visual.text(
          `${role} ${numOfCreeps}/${neededCreeps}`,
          1,
          14.5 + indent + antiIndet,
          { color: color, font: 0.8, align: LEFT }
        );
      } else {
        color = "orange";
        this.visual.text(
  ...
  listOfNotLimitedRoles.forEach((role, indent) => {
    ...
    this.visual.text(
      `${role} ${_.sum(creepsInRoom, (c) => c.memory.role == role)}`,
      1,
      endIndex,
      { color: "#B5B5B5", font: 0.8, align: LEFT }
    );
    endIndex++;
  });
  endIndex++;
  ...
  spawnsInRoom.forEach((spawn, indent) => {
    ...
    if (spawn.spawning)
      this.visual.text(
        ...
        41,
        3.6 + 1.9 * indent,
        { color: "green", font: 0.7, align: LEFT }
      );
    else
  ...
  if (this.storage != undefined) {
    ...
    storageResources.forEach((resource, indent) => {
      ...
      if (displayData.displayGraph) {
        ...
        if (!_.isArray(resourceArray) || resourceArray.length == 0)
          resourceArray = [[Game.time, resourceNum]];
    ...
    if (displayData.displayGraph) {
      for (let resourceName in displayData.lastStorage) {
        ...
        if (resourceArray.length > 2)
          ticksPerBlock = ticksPerBlock.sort((a, b) => a[0] - b[0]).slice(0, 2);
      ...
      for (let i = 39; i <= 48; i += 0.6)
        this.visual.line(1, i, 22, i, { color: gray, opacity: 0.02 });
      for (let i = 1; i < 22; i += 0.6)
        this.visual.line(i, 48, i, 39, { color: gray, opacity: 0.02 });
  ...
  if (this.terminal != undefined) {
    ...
    if (
      Memory.rooms[this.name].terminal &&
      _.isEqual(Object.keys(Memory.rooms[this.name].terminal), [
        "enabled",
        "autoSell",
        "from",
        "to",
      ...
      if (terminalMemory.enabled) {
        if (_.some(terminalMemory.autoSell, { enabled: true })) {
          ...
          terminalMemory.autoSell.forEach((autoSell) => {
            if (autoSell.enabled) {
              ...
              endIndex++;
              this.visual.text(
                `MP: ${autoSell.minPrice}, MD: ${autoSell.maxDistance}`,
                42.2,
                endIndex,
                { color: "#DEDEDE", font: 0.8, align: LEFT }
              );
        ...
        if (_.some(terminalMemory.to, { enabled: true })) {
          ...
          terminalMemory.to.forEach((to) => {
            if (to.enabled) {
              this.visual.resource(
                to.dealData != undefined
                  ? to.dealData.resourceType
                  : to.resource,
                41.8,
                endIndex,
                0.5
              ...
              endIndex++;
              let moreInfo =
                to.order && to.resource != undefined
                  ? `order SELL: ${to.resource}`
                  : `${to.dealData.roomName} ${to.dealData.price}$`;
        ...
        if (_.some(terminalMemory.from, { enabled: true })) {
          ...
          terminalMemory.from.forEach((from) => {
            if (from.enabled) {
              this.visual.resource(
                from.dealData != undefined
                  ? from.dealData.resourceType
                  : from.resource,
                41.8,
                endIndex,
                0.5
              ...
              endIndex++;
              let moreInfo =
                from.order && from.resource != undefined
                  ? `order BUY`
                  : `${from.dealData.roomName} ${from.dealData.price}$`;
              moreInfo +=
                from.toStructure != undefined ? ` to ${from.toStructure}` : "";
              this.visual.text(moreInfo, 42.2, endIndex, {
                color: "#DEDEDE",
                font: 0.8,
                align: LEFT,
  ...
  let energyInTheRoomCollor =
    this.energyAvailable == this.energyCapacityAvailable
      ? "green"
      : this.energyAvailable >= this.energyCapacityAvailable / 3
      ? "orange"
      : "red";
  ...
  endIndex++;
  let energyPersentage =
    Math.round(
      (this.energyAvailable / this.energyCapacityAvailable) * 100 * 10
    ) / 10;
  this.visual.text(
    `${this.energyAvailable}/${this.energyCapacityAvailable}  (${energyPersentage}%)`,
    41,
    endIndex,
    { color: energyInTheRoomCollor, font: 0.8, align: LEFT }
  );
  endIndex++;
  endIndex++;
  ...
  endIndex++;
  let controllerProgressPersentage =
    Math.round(
      (this.controller.progress / this.controller.progressTotal) * 100 * 10
    ) / 10;
...
Room.prototype.options = function (name, data) {
  if (name != undefined && data != undefined) {
    ...
    return `ERROR: Expected option name and data that will be assigned to a given option name`;
};
...
Room.prototype.globalOption = function (name, data, roomList = false) {
  if (name != undefined && data != undefined) {
    if (!roomList)
      roomList = Object.keys(Game.rooms).filter(
        (roomName) =>
    ...
    if (roomList.length == 1)
      return `Set room ${roomList.join(", ")} option ${name} to ${data}`;
    else if (roomList.length > 1)
      return `Set rooms ${roomList.join(", ")} option ${name} to ${data}`;
...
Room.prototype.globalHardOption = function (name, data, roomList = false) {
  if (name != undefined && data != undefined) {
    ...
    if (roomList.length == 1)
      return `Set room ${roomList.join(", ")} hard option ${name} to ${data}`;
    else if (roomList.length > 1)
      return `Set rooms ${roomList.join(", ")} hard option ${name} to ${data}`;
    else return `Sorry, but no rooms found`;
  } else
    return `ERROR: Expected option name and data that will be assigned to a given option name`;
};
