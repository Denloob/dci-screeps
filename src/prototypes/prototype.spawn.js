var listOfRoles = ['harvester', 'collector', 'deffender', 'healer', 'attacker', 'dismantler', 'claimer', 'upgrader', 'TM', 'MH', 'repairer', 'builder', 'wallRepairer', 'scout'];

// create a new function for StructureSpawn
StructureSpawn.prototype.spawnCreepsIfNecessary = function () {
  /** @type {Room} */
  let room = this.room;
  if (!_.isObject(this.memory))
    this.memory = {
      minCreeps: { harvester: 2, collector: 0, upgrader: 2, builder: 1, repairer: 0, wallRepairer: 0, MH: 1, TM: 1 },
      minLongDistanceHarvesters: {},
      minLongDistanceCollectors: {},
      minDeffenders: { attacker: 2 },
    };
  else
    ['minCreeps', 'minLongDistanceHarvesters', 'minLongDistanceCollectors', 'minDeffenders'].forEach((key) => {
  ...
  for (let spawnName in Game.spawns) {
    ...
    if (spawn.spawning) spawnsSpawning.push(spawn);
  }
  ...
  if (name == undefined) {
    for (let role of listOfRoles) {
      if (role == 'deffender') {
        // TODO TEST IF WORKS AND FIX -- 09.12
        ...
        if (this.memory.minDeffenders.attacker && numOfRoomDeffenders < this.memory.minDeffenders.attacker) {
          ...
          if ((hostileCreeps.length && _.sum(hostileCreeps, (hc) => hc.owner.username != 'Invader')) || hostileCreeps.length > 4) {
            ...
            Game.notify(
              `[${time}] spawn in ${this.room.name} spawned a deffender because hostile creeps found with code ${name}. first creep found: ${hostileCreeps[0].name} of ${hostileCreeps[0].owner.username}`
            );
            console.log(
              `spawn in ${this.room.name} spawned a deffender because hostile creeps found with code ${name}. first creep found: ${hostileCreeps[0].name} of ${hostileCreeps[0].owner.username}`
            );
            break;
          } else continue;
        } else continue;
      ...
      else if (role == 'healer' && this.memory.healTarget != undefined) {
        if (this.memory.healTarget.length < 3) console.log(`healTarget length is wrong at ${this.name}`);
      ...
      // if no order was found, check other roles
        ...
        if (role == 'collector') {
          ...
          if (roomExtractor != undefined && roomMineral != undefined && roomMineral.mineralAmount > 0) name = this.createCustomCreep(maxEnergy, role);
          else continue;
        } else if (role == 'TM') {
          if (
            Memory.rooms &&
            Memory.rooms[this.room.name] &&
            Memory.rooms[this.room.name].terminal &&
            _.isEqual(Object.keys(Memory.rooms[this.room.name].terminal), ['enabled', 'autoSell', 'from', 'to'])
          ) {
  ...
  if (
    ...
    this.memory.buildRoom == undefined &&
    this.memory.dismantleAttackRoom == undefined &&
    this.memory.attackRoom == undefined
  ) {
...
// create a new function for StructureSpawn
StructureSpawn.prototype.createNotWorkerCreep = function (energy, role, numberOfWorkParts = 0, additionalMemory = NaN) {
  // create a balanced body as big as possible with the given energy
  let body = [];
...
StructureSpawn.prototype.createLongDistanceHarvester = function (energy, numberOfWorkParts, home, target, sourceIndex) {
  ...
  let newCreep = this.spawnCreep(body, name, {
    memory: {
      role: 'LDH',
      home: home,
      target: target,
...
// create a new function for StructureSpawn
StructureSpawn.prototype.createLongDistanceCollector = function (energy, home, target) {
  let body = [];
...
StructureSpawn.prototype.createAttacker =
  ...
  function (energy, target, numberOfAttackParts, tough, id) {
    let body = [];
    for (let i = 0; i < numberOfAttackParts && body.length < 49; i++) {
      body.push(ATTACK);
      body.push(MOVE);
    }
    ...
    if (tough) {
      ...
      for (let i = 0; i < numberOfParts && body.length < 49; i++) {
        body.push(TOUGH);
        body.push(MOVE);
      }
    }
...
StructureSpawn.prototype.createHealer =
  ...
  function (energy, target, numberOfHealParts, tough, id) {
    let body = [];
    for (let i = 0; i < numberOfHealParts && body.length < 49; i++) {
      body.push(HEAL);
      body.push(MOVE);
    }
...
StructureSpawn.prototype.createDismantler =
  ...
  function (energy, target, numberOfWorkParts, tough, id) {
    let body = [];
    for (let i = 0; i < numberOfWorkParts && body.length < 49; i++) {
      body.push(WORK);
      body.push(MOVE);
    }
    energy -= (BODYPART_COST[MOVE] + BODYPART_COST[WORK]) * numberOfWorkParts;
...
// create a new function for StructureSpawn
StructureSpawn.prototype.createClaimerCreep = function (target, numberOfClaimParts, reserve = false) {
  let body = [];
...
// create a new function for StructureSpawn
StructureSpawn.prototype.createMinerCreep = function (sourceId) {
...
StructureSpawn.prototype.attackRoom = function (target, dismantlers, attackers, healers, tough, forceAttack = false) {
  ...
  Memory.rooms[this.room.name].attackRoom.push({
    target: target,
...
StructureSpawn.prototype.processAttacks = function () {
  if (Memory.rooms && Memory.rooms[this.room.name] && _.isArray(Memory.rooms[this.room.name].attackRoom))
    Memory.rooms[this.room.name].attackRoom.forEach((task) => {
      ...
      if (
        Game.rooms[task.target] != undefined &&
        !(Game.rooms[task.target].controller == undefined || Game.rooms[task.target].controller.safeMode == undefined || Game.rooms[task.target].controller.safeMode <= 200)
      )
        return;
      ...
      if (numOfHealers < task.healers && (numOfAttackers > 0 || numOfDismantlers > 0))
        this.spawnHealer(
          _.filter(
            Game.creeps,
            (c) => c.memory.id == task.id && (c.memory.role == 'attacker' || c.memory.role == 'dismantler' || c.memory.hardRole == 'attacker' || c.memory.hardRole == 'dismantler')
          )[0].name,
          Math.floor(this.room.energyCapacityAvailable / 300) - 1,
          task.tough,
          task.id
        );
