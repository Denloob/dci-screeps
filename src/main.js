...
function mainLoop() {
  ...
  for (const roomName in Game.rooms) {
    ...
    if (
      _.isObject(_.get(Memory, `rooms["${roomName}"]`)) &&
      Object.keys(Memory.rooms[roomName]).length == 0
    ) {
    ...
    if (
      _.get(room, `controller.my`, false) &&
      _.isObject(Memory.rooms[roomName])
    ) {
      if (!_.isArray(Memory.rooms[roomName].boostCreeps))
        Memory.rooms[roomName].boostCreeps = [];

      [...Memory.rooms[roomName].boostCreeps].forEach(
        ([creepName, resource], index) => {
          if (!_.isString(creepName)) {
            statsConsole.log(
              "err, for creepName, resource of boostCreeps: creepName is not a string"
            );
            return;
          }
          if (!_.isString(resource)) {
            statsConsole.log(
              "err, for creepName, resource of boostCreeps: resource is not a string"
            );
            return;
          }
          ...
          if (creep.memory.hardRole == undefined)
            creep.memory.hardRole = creep.memory.role;
          if (creep.memory.role != "boosting") creep.memory.role = "boosting";
          if (!Object.keys(Game.structures).includes(creep.memory.labTarget))
            creep.memory.labTarget = creep.pos.findClosestByPath(
              FIND_MY_STRUCTURES,
              {
...
function roomLogic(roomName) {
  if (
        ...
        energy: [
          [
            ...
            Game.rooms[roomName].storage
              ? Game.rooms[roomName].storage.store.energy
              : 0,
          ],
        ],
      },
    ...
    if (typeof Memory.hardMemory !== "object")
      Memory.hardMemory = { rooms: {} };
    ...
    if (typeof Memory.rooms[roomName].displayData !== "object")
      Memory.rooms[roomName].displayData = defaultdisplayData;
    if (typeof Memory.hardMemory.rooms[roomName] !== "object")
      Memory.hardMemory.rooms[roomName] = defaultHardMemory;
    ...
    if (
      ...
      !_.isEqual(
        ...
        Object.keys(defaultdisplayData).sort()
      )
    ) {
    ...
    if (
      typeof hardMemory === "object" &&
      !_.isEqual(
        Object.keys(hardMemory).sort(),
        Object.keys(defaultHardMemory).sort()
      )
    ) {
      statsConsole.log(
    ...
    if (typeof displayData === "object") {
      if (
        displayData.fill != "transparent" &&
        !displayData.fill.startsWith("#")
      )
        displayData.fill = `#${displayData.fill}`;

      if (displayData.enabled) Game.rooms[roomName].displayData();

      if (!_.isEqual(roomMemory.displayData, displayData))
        roomMemory.displayData = displayData;
    }
  }
}
...
function roomCallback(roomName, matrix) {
  ...
  let hostiles = room.find(FIND_HOSTILE_CREEPS, {
    filter: (c) =>
      _.some(c.body, (b) => b.type == ATTACK || b.type == RANGED_ATTACK),
  });
  hostiles.forEach((h) => {
